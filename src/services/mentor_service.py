...
def upsert_registration(team_id: str, user_id: str, role: str, interests: list) -> None:
...
def clear_pair(team_id: str, user1_id: str, user2_id: str) -> None:
    ...
    col.update_one({"user_id": user1_id}, {"$set": {"matched_with": None, "matched_at": None}})
    col.update_one({"user_id": user2_id}, {"$set": {"matched_with": None, "matched_at": None}})
...
def get_all_pairs(team_id: str) -> list[tuple[str, str]]:
    """Return (mentor_id, mentee_id) for all currently matched pairs."""
    col = _get_col(team_id)
    mentors = list(col.find({"role": "mentor", "matched_with": {"$ne": None}}, {"_id": 0}))
    return [(m["user_id"], m["matched_with"]) for m in mentors]


def run_matching(team_id: str, mentors: list, mentees: list) -> list[tuple[str, str, list]]:
    """
    Greedy match mentors to mentees by shared interest count.
    ...
    # Pair any leftovers that had no shared interests
